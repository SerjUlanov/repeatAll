...
function t2(min, max, block) {
    block.textContent = Math.floor(Math.random() * (max - min + 1)) + min;  //Максимум и минимум включаются
}
...
function t3(min = 0, max = 100) {
    out3.textContent = Math.floor(Math.random() * (max - min + 1)) + min;
}
...
function t11() {
    ...
    // out11.textContent = ar.map(i => x += i, x = 0).reverse()[0];
}

document.querySelector('.b-11').addEventListener('click', function () {
    t11(33, 22, 44, 11, 55, 66, 11, 66, 12);
})
...
function t12(...num) {
    out12.textContent = num.map(i => x += i, x = 0).reverse()[0];
}

document.querySelector('.b-12').addEventListener('click', function () {
    t12(33, 22, 44, 11, 55, 66, 11, 16);
})
